...
const handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  if (command === 'play' || command === 'play2' || command === 'playvid') {
    ...
    try {
      ...
      if (!audio?.data?.url) throw 'No se recibió URL de audio';
    } catch {
      try {
        ...
        try {
          ...
          throw "No se pudo obtener el audio desde ninguna API.";
        }
      }
    }

    await conn.sendMessage(m.chat, {
      audio: { url: audio.data.url },
      mimetype: 'audio/mpeg',
    ...
    try {
      ...
      if (!video?.data?.url) throw 'No se recibió URL de video';
    } catch {
      try {
        ...
        try {
          ...
          throw "No se pudo obtener el video desde ninguna API.";
        }
      }
    }
